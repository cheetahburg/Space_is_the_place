...
function tte_ext_input_draw(){
    ...
    surface_set_target( tte_surface );
        ...
        for( l=0; l<ds_list_size( lines ); l++ ) {
            ...
            if( selVisible ) {
                ...
                if( l >= selStartLine && l <= selEndLine ) {
                    if( l == selStartLine && l == selEndLine ) draw_rectangle(
                        ...
                        ch + max( mh, string_height( lines[| l] )*2 )-1 - scrolly,
                        false
                    );
			...
			if typ=="password"{
				tmp="";
				for (var i=0;i<string_length(lines[| l]);i++){
					tmp+="*";
				}
			}else{
				tmp=lines[| l];
            ...
            //draw_text( -scrollx, ch - scrolly, tmp );
			draw_text_transformed_color(-scrollx, ch - scrolly, tmp,1,1,0,c_lime,c_lime,c_lime,c_lime,1);
